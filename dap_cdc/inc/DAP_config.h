...
#define PIN_TDI_IOCON           LPC_IOCON->PIO0[17]
...
static __inline uint32_t PIN_SWCLK_TCK_IN  (void) {
    return LPC_GPIO->B[0][PIN_SWCLK_IN_BIT] & 0x1;
}
...
static __inline uint32_t PIN_SWDIO_TMS_IN  (void) {
    return LPC_GPIO->B[0][PIN_SWDIO_IN_BIT] & 0x1;
}
...
#if (DAP_JTAG != 0)
    return LPC_GPIO->B[0][PIN_TDI_IN_BIT] & 0x1;
#else
...
#if (DAP_JTAG != 0)
    return LPC_GPIO->B[0][PIN_TDO_IN_BIT] & 0x1;
#else
  return (0);   // Not available
#endif
}
...
static __inline uint32_t PIN_nRESET_IN  (void) {
    return LPC_GPIO->B[0][PIN_nRESET_IN_BIT] & 0x1;
}
